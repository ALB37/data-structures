...
def test_insert_pop(new_heap):
    """Test that unique inserted item is popped before all other items."""
    val = MAX_INT if MAX else MIN_INT
    new_heap.instance.insert(val)
    assert new_heap.instance.extractMin() == val
...
def test_pop(new_heap):
    ...
    if new_heap.pop_value is None:
        pytest.skip()
    assert new_heap.instance.extractMin() == new_heap.pop_value


def test_pop_unchanged(new_heap):
    """Test pop value is unchanged after inserting a lesser/ greater value."""
    if new_heap.pop_value is None:
        pytest.skip()
...
def test_pop_sequence(new_heap):
    ...
    output = [new_heap.instance.extractMin() for _ in sequence]
